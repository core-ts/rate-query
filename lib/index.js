...
var SqlRateRepository = (function () {
  function SqlRateRepository(db, table, attributes, buildToSave, max, infoTable, id, rate, count, score) {
    this.db = db;
    this.table = table;
  ...
  SqlRateRepository.prototype.getRate = function (id, author, ctx) {
    return this.db.query("select * from " + this.table + " where id = " + this.db.param(1) + " and author = " + this.db.param(2), [id, author], this.map, undefined, ctx).then(function (rates) {
  ...
  SqlRateRepository.prototype.insert = function (rate, newInfo) {
    ...
    if (stmt) {
      if (newInfo) {
        var query = this.insertInfo(rate.rate);
        var s2 = { query: query, params: [rate.id] };
        return this.db.execBatch([s2, stmt], true);
      }
      else {
        var query = this.updateNewInfo(rate.rate);
        var s2 = { query: query, params: [rate.id] };
        return this.db.execBatch([s2, stmt], true);
      }
    }
  ...
  SqlRateRepository.prototype.update = function (rate, oldRate) {
    ...
    if (stmt) {
      var query = this.updateOldInfo(rate.rate, oldRate);
      var s2 = { query: query, params: [rate.id] };
      return this.db.execBatch([s2, stmt], true);
    }
    else {
