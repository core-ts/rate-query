...
var SqlRateRepository = (function () {
  function SqlRateRepository(db, table, attributes, buildToSave, max, infoTable, count, score, rate, rateField, authorCol, id, idField, idCol) {
    this.db = db;
    this.table = table;
...
var SqlCommentRepository = (function (_super) {
  ...
  function SqlCommentRepository(db, table, attrs, parent, idField, authorField, idCol, authorCol, col, author, time, id) {
    ...
    _this.idCol = (idCol && idCol.length > 0 ? idCol : 'id');
    _this.authorCol = (authorCol && authorCol.length > 0 ? authorCol : 'author');
